...
export async function writeGeneratedProject(projectId: string, blueprint: Blueprint): Promise<GeneratedFileRecord[]> {
  ...
  for (const comp of blueprint.components) {
    ...
    created.push({ type: 'component', relativePath: rel });
  }
  if (barrelLines.length) {
    ...
    created.push({ type: 'barrel', relativePath: barrelRel });
  }
  ...
  for (const page of blueprint.pages) {
    ...
    await fs.writeFile(path.join(base, rel), code, 'utf8');
    created.push({ type: 'page', relativePath: rel });
  }
  ...
  for (const api of blueprint.apiRoutes) {
    ...
    await fs.writeFile(path.join(base, rel), code, 'utf8');
    created.push({ type: 'api', relativePath: rel });
  }
  ...
  if (blueprint.prismaModels?.length) {
    ...
    created.push({ type: 'prisma', relativePath: rel });
  }
  return created;
}
